...
extension FieldVariableView: DropdownOptionsViewControllerDelegate {
                                            ...
                                            didSelectOptionIndex optionIndex: Int)
  {
    ...
    if (value == "add") {
      ...
      fieldVariableLayout?.fieldVariable.nameManager?.requestRemovalForName((fieldVariableLayout?.variable)!)
    } else {
...
extension FieldVariableView: VariableNameViewControllerDelegate {
  public func variableNameViewController(_ viewController: VariableNameViewController, didChangeName name: String?) {
    guard let newName = name else {
      return
    }

    fieldVariableLayout?.fieldVariable.nameManager?.addName(newName)
    fieldVariableLayout?.changeToVariable(newName)
    //    fieldVariableLayout?.fieldVariable.nameManager?.renameName((fieldVariableLayout?.variable)!, to: newName)
  }
}
